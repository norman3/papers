...
- 즉, `Mul` 연산에 대해 하나의 글로벌 변수 `g_register_mul` 을 생성하고 이 객체가 생성될 때 생성자에서 `Mul` 을 글로벌 레지스터에 등록하게 된다.
- 꽤나 괜찮은 전략이지만 한가지 문제가 있다.
    - 만약 TensorFlow를 라이브러리화 하거나 컴파일 할 때에 `g_register_mul` 등은 선언만 있지 실제 작업을 하지 않는다.
    - 따라서 Linker는 Binary 작성시 사용하지 않는 `g_register_mul` 객체를 아예 배제하게 된다.
    - 하지만 나중에 그래프를 구성할 때 `Mul` 을 사용하게 되면 이를 글로벌 레지스터에서 찾게 된다.
    - 이 때 `g_register_mul` 이 코드에서 제거되어 있다면 생성자를 호출하지 않을 것이고 따라서 등록이 되어 있지 않게 된다.
...
- 이 프로토콜의 문제는 버전마다 문제가 발생한다는 것인데, 사용자가 작성한 pb 파일을 어떤 버전으로 생성했느냐에 따라 문제가 달라지게 된다.
    - 예를 들어 `protobuf-3.0.1.a` 로 생성한 pb 를 해석해야 하는데 현재 장비에는 protobuf-3.0.0 이 설치되어 있는 경우 등.
    - 따라서 동일 버전을 사용하도록 하자.
- 또 다른 문제는 protobuf 기술 파일이 Build 단계에서 요구된다는 것이다.
    - 이 과정은 꽤나 복잡한데 먼저 protobuf 정의를 해석해서 헤더를 생성한 뒤 진짜 코드가 컴파일되는 것이다.
- 일단 pb-version 1을 사용하는 경우 TensorFlow에서 사용하는 것은 포기해라.
    - 이게 무슨말인고 하니 이미 사용하고 있는 응용 프로그램 에제가 pb-version-1 을 사용중인데,
    - 여기에 TensorFlow를 추가로 모듈에 합칠 수 없다는 이야기다.
    - TensorFlow 는 version 2 이상 사용 가능하다.
...
- 보통 작업은 다음과 같이 진행될 것이다.
    - 먼저 여러분이 만든 모델을 로드한다.
    - 입력 데이터를 적당하게 전처리한 뒤 모델 입력에 넣는다. (예를 들면 이미지 등)
    - 그리고 모델을 구동해서 결과는 얻는다. (예를 들어 레이블 정보)
...
- 사실 PC 레벨에서는 C++ 만 이용해서 TensorFlow 모듈을 다 구션할 수 있다.
...
- 하지만 federated learning 이라는 방법도 있다.
    ...
    - 실제 구현체의 예는 Google Keyboard App. 이다.
- 하지만 device에서 학습을 한다는 것 자체는 무척이나 어려운 일.
    - TensorFlow는 C++ API 자체에서 auto gradient 기능이 제공되는 것이 아니다.
...
- Runtime 에서 요구되는 최소 리소스 사양은 어떻게 될까?
- 정확히 이야기하기는 어려우나 일단 DSP 나 microcontroller 를 사용하는 Embedding 환경에서는 확실히 돌릴 수 없다.
- 게다가 성능(속도나 연산 가능성 여부)이 주요한 요소이므로 사용자의 어플리케이션 수준에 따라 최소 리스스량은 달라진다.
- 따라서 정량적인 한계즌 찾기 어렵고, 그냥 제일 간단한 지표로 모델의 FLOPs를 생각해 볼 수 있다.
    - 즉, 주어진 모델을 돌릴 때 몇 FLOP가 소요되는가를 묻는 것이다.
...
- *NodeDef*
    ...
    - 고유한 이름을 가지며 다음과 같은 정보를 저장하고 있다.
        ...
        - 연산자 제어를 위한 속성들
        - 독특하게 `Const` 라는 연산자는 값을 저장하고 있다.
            - 보통 정수나 실수, 문자열들을 포함한다.
            - 그리고 Tensor 타입 저장도 가능하다.
            - 중요한 것은 이 정보는 NodeDef 에 저장된다.
- *CheckPoint*
    - 데이터를 저장하는 다른 방법은 `Variable` 연산자를 사용하는 것이다.
        - `Const` 연산자와는 다르게 `NodeDef` 에 이 값을 저장하지 않는다.
        ...
        - 따라서 time-critical 한 연산이라 할 수 있다.
        - 이를 위해 실제 저장을 해야 하는 파일의 포맷은 아주 빠르고 유연한 형태로 구조화되어야 한다.
    - 최종적으로 모델은 여러 버전의 체크 포인트 파일을 가지게 된다.
        - 이 파일은 별도의 API로도 접근 가능하다.
    - 저장되는 형식은 다음과 같다.
...
- *GraphDef*
    - `GraphDef` 는 `NodeDef` 의 리스트를 담고 있다.
    - 이는 실행을 하기 위한 연산 그래프를 정의하고 있다.
    - 학습 과정 중 완전한 그래프 구성을 위해서는 실제 weight 값도 필요하므로 check-point 파일도 함께 로드해서 사용해야 한다.
        - 빠른 학습을 위해 취한 모델이지만 좀 불편하기는 하다. (2 종류의 저장 파일이 필요하다.)
    - 이후 이를 편리하게 사용하기 위해 약간의 꼼수를 사용한다.
        ...
        - 학습이 끝난 뒤 모든 `Variable` 노드를 `Const` 노드로 바꾸어 그래프를 저장한다.
            - 이렇게 하면 모든 weight 는 Check-Point 파일이 아닌 `NodeDef` 에 저장되게 된다.
...
- TensorFlow가 outut-format을 하도 자주 바꾸기 때문에 기타 드물게 사용되는 옵션들은 쓰지 말도록 하자.
...
- weight 업데이트를 위해서는 반드시 필요한 노드이다.
- 하지만 이런 노드들은 inference 시에는 더 이상 필요하지 않다.
- 이 때 사용할 수 있는 기능이 `strip_unused_nodes` 이다. (Graph Transform Tool 에 포함되어 있다.)

-  입출력 노드를 이해하는 것이 매우 중요한데 Graph Transform 시에 이 입출력 노드를 바탕으로 Path 를 만들기 때문이다.
...
- 모바일 환경을 생각해보자.
    - 대부분의 데이터를 모바일 device 의 메모리로부터 입력 데이터를 받을 수 있다.
        - 예를 들면 Inception-v3 모델의 경우 `DecodeJpeg` 연산자를 이용해서 파일로 부터 데이터를 입력받게 된다.
        - 그런다음 `BilinearResize` 연산자로 이미지 resize 후에 입력을 받게 된다.
...
- 학습한 모델을 바로 모바일 디바이스에 넣고 돌릴 때 가장 많이 보는 에러는
- 바로 `No OpKernal was registered to support Op` 이다.
- 이 때 가장 먼저 해야 할 일은 `strip_unused_nodes` 이다.
    - 에러가 가는 Op가 날라가기만 하면 해결되는 문제가 아니던가.
...
- 연산자(Operation)는 구현시 크게 2가지 파트로 나뉘게 된다.
    - *op definition* : 연산자에 대한 signature 라고 생각하면 된다. 크기가 작으므로 모두 저장되게 되어 ㅣㅇㅆ다.
...
- device에 모델을 올리려면 모델의 크기가 주요한 이슈가 된다.
...
- 파일 크기를 살펴보면 최초 파일의 1/4 정도로 줄게 된다. (처음 크기는 23M라한다.)
- 일반적으로 `float` 타입의 데이터는 잘 압축되지 않는다.
    - 실수 값이 조금만 차이가 나도 실제 저장되는 형식 데이터는 크게 다르게 되므로 압축이 어려워진다.
    - `round_weights` 등은 단순하게 round 하는 방식이지만 압축율을 크게 높일 수 있다.
...
- device 마다 처리되는 속도가 다르다.
    - 2016년 상급 폰이 대충 초당 20 billion FLOPs 정도 나온다.
    - 따라서 모델이 대충 10 billion FLOPs 정도 성능이 나온다면 이 폰에서는 초당 2장 처리가 가능하다.
...
- 그런 경우 2개의 Tensor만을 추가함으로써 (min/max용) 원래 값들을 int8 타입으로 변경 가능하다.
...
- 다른 문제로 min과 max 값이 같다거나 min이 max 보다 더 큰 상황이 생길 수 있다.
    ...
    - 해결 방법은 1을 더한 값을 인코딩한다. (대충 될 것 같은데 귀찮아서 생각 정지.)
...
- 일단 다음 연산에 대해서만 수행한다. (이건 업데이트 될 수도 있는 내용이다.
    - `BiasAdd`, `Concat`, `Conv2D`, `MatMul`, `Relu`, `Relu6`, `AvgPool`, `MaxPool`, `Mul`

- 이것 만으로 Inception 등은 충분히 표현 가능하다.
