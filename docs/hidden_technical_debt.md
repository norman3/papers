...
- 이는 새로운 피쳐 \\(x\_(n+1)\\) 을 추가한다거나 혹은 임의의 피쳐 \\(x\_j\\) 를 제거할 때에도 동일합니다.
- 모든 입력은 서로 독립적이지 않습니다. 이를 우리는 CACE 라고 부릅니다.
...
- 이를 통합할 수 있는 한가지 방법은 앙상블(ensemble)을 도입하는 것입니다.
    - 이 방법은 특정한 경우에는 잘 동작합니다
        - 문제 정의상 문제가 서브 태스크로 나뉘어질 수 있는 경우
    - 하지만 많은 경우 앙상블은 잘 동작합니다. 각 모델의 에러는 서로 연관성이 없기 때문입니다.
    - 이러한 이유로 앙상블 모델은 강한 얽힘을 가지게 됩니다.
        - 개별 구성 요소를 개선하더라도 성능이 오히려 나빠질 수 있습니다. (다른 컴포넌트들이 강하게 연관되어 있는 경우에)
- 두 번째 대안으로 발생할 수 있는 예측 행동을 찾는 것에 주력하는 것입니다.
    - 고차원 정보를 시각화할 수 있는 Tool 을 활용하여 다차원 공간에서의 효과를 슬라이싱합니다.
...
- 게다가 이 문제는 숨겨진 피드백 루프 (hidden feedback loop) 를 만들어냅니다. (뒤에 설명합니다.)
...
- 하지만 ML 시스템에선 데이터 의존성 문제가 더 큰 문제입니다.
    - 게다가 더 발견하기 어렵습니다.
- 코드에 의한 의존성 부채는 정적 분석 Tool 등을 활용하여 제거할 수 있습니다. 하지만 데이터에 의한 의존성 부채는 이런 Tool 들이 없습니다.
...
- 이에 대한 보완 전략은 불완전한 데이터에 대한 버저닝 관리입니다.
    ...
    - 물론 이러한 버전 관리는 내부적으로 부실하게 관리될 가능성이 있고 시간이 지남에 따라 동일한 신호 정보를 포함한 여러 버전을 만들어낼 수 있다는 단점이 있습니다.
        - 즉, 동일한 정보를 가진 여러 클러스터가 발생하더라도 이를 유지해야 하는 비용이 발생합니다.
...
- 하지만 데이터 의존성에 대해서는 이에 대한 도구나 경험이 부족합니다.
    - 그럼에보 불구하고 오류 확인, 사용자 추적, 마이그레이션, 업데이트 시행시 필수적인 부분입니다.
...
- 이 때 다양한 오픈소스 패키지를 사용하게 되는데 이런 경우 접착코드 패턴이 생겨납니다.
    - 데이터 입출력시 막대한 양의 지원 코드가 생겨나기 쉽습니다.
- 접착제 코드는 코드를 특정 패키지에 고정시키는 경향이 있기 때문에 장기적으로는 비싼 비용을 치르게 됩니다.
...
- 이 예시 중 가장 유명한 일화는 45분만에 4억6천6백만 달러의 손실을 본 Knight Capital 시스템입니다.
...
- Zheng 의 연구에 따르면 ML 추상화의 정도를 DB 시스템과 비교하여 어떠한 ML 시스템도 DB에서 성공한 추상화 시스템에 적합하지 않다는 것을 확인합니다.
- 데이터 스트림, 모델, 추론을 표현할 수 있는 적절한 인터페이스는 무엇일까요?
- 특히 분산 학습을 위한 추상화 모델은 존재하기나 할까요?
    - 가장 널리 알려진 분산 알고리즘인 MR 방식이 ML에서 자주 활용되는 이유가 사실은 강력한 분산 학습 추상화가 존재하지 않기 때문이라는 시각도 있습니다.
...
- 소프트웨어 엔지니어링에서는 시스템이나 컴포넌트에 존재하는 문제들을 냄새(smell)로 표현합니다.
- 여기서는 ML 시스템에서 발생 가능한 냄새를 정의합니다.

#### POD 타입 냄새 (Plain-Old-Data Type Smell)

- ML 시스템에서 사용되는 정보는 모두 POD 타입의 정수나 실수로 인코딩됩니다.
...
- 제공된 언어로 시스템의 특정 부분을 구현하려는 경우가 많습니다.
...
- 대형 시스템에는 사용되는 기능, 대에터 선택, 다양한 학습 옵션, 암묵적인 전처리, 검증 방법 등 다양한 영역에서 설정 정보를 사용하게 됩니다.
- 연구원과 엔지니어는 보통 설정 정보에 관련된 내용들을 나중으로 미룹니다.
    - 실제로 설정 정보의 검증 또는 테스트는 중요하지 않을 수도 있습니다.
- 성숙한 시스템에서의 설정 정보의 코드 수는 실제 구동 코드보다도 더 많을 수 있습니다.
    - 이 경우 각 설정 정보에는 실수가 있을 수 있습니다.
- 예를 들어 봅시다.
...
- 반면 단순하게 입력 데이터와 상관 없이 발생되는 방식의 널모델로 레이블에 대한 평균값을 예측하는 방법은 정석은 아닙니다.
...
- 이러한 프로세스는 절저한 모니터링, 테스트 및 요구사항 만족을 위한 수준을 충족해야 합니다.
...
- ML 시스템에서 데이터가 코드를 대체하고 이 코드는 테스트되어야 한다고 할 때 테스트를 위한 데이터로 테스트를 하는 것은 명확해 보입니다.
- 기본적인 체크는 유용하지만 복잡한 테스트는 입력 분포를 변화시킵니다. (???)
